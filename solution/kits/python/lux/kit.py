...
def obs_to_game_state(step, env_cfg: EnvConfig, obs):
    ...
    for agent in obs["teams"]:
        ...
        teams[agent] = Team(team_data["team_id"], agent=agent, faction=team_data["faction"])
        teams[agent].factories_to_place = team_data["factories_to_place"]
        teams[agent].factory_strains = team_data["factory_strains"]
        teams[agent].place_first = team_data["place_first"]
        teams[agent].bid = team_data["bid"]

        teams[agent].init_metal = team_data["metal"]
        teams[agent].init_water = team_data["water"]
