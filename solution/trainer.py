    ...
    for player_, prediction in predictions.items():
        for unit_type, policy in prediction.items():
            ...
            if not isinstance(policy, dict):
                policy_loss += vs["reward"] * (real_policy * torch.log(policy + 1e-9)).sum()
...
def value_policy_loss(predictions, v, vs, policies):
    ...
    for player, prediction in predictions.items():
        for unit_type, policy in prediction.items():
            ...
            if not isinstance(policy, dict):
                policy_loss += (real_policy * torch.log(policy + 1e-9)).sum()
    return v_loss - policy_loss


class Trainer:
    def __init__(self, weights_path, model, dataset = None, weight_decay = 1e-5, n_games = 200):
        if dataset is None:
            dataset = DatasetBuilder(n_games=n_games)
